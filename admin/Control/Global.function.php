...
function safehtml($html) {
	if(is_array($html)) {
		...
		//addslashes是进行转义操作。。。在init.php里面已经进行了判断，如果get_magic_quotes_gpc没有开启就使用addslashes
		//强制进行转义，也就是加\。两者不能同时使用!!!

		//Convert special characters to HTML entities  htmlspecialchars 比如&变为&amp

		// strip_tags可以方便的去除html标签、、
...
function csetcookie($name,$value,$life = 0) {
	//默认携带这个sessionid的cookie的存活时间是0也就是浏览器关掉后cookie即失效。这正是session存活时间的特点。
	//另外从配置文件中读取了前缀来作为cookie的前缀。
	setcookie("jia"."_".$name,$value,$life?(time()+$life):0,"/");
...
function cgetcookie($name) {
	//获取session所对应的session_name。得到全局变量中cookie的pre为cart，也就是去获得cart_sess
	return isset($_COOKIE["jia"."_".$name])?$_COOKIE["jia"."_".$name]:"";
}
...
function url($pagetype,$model,$action = '',$other = '',$html = false) {
	...
	// var_dump($url);
	//将一个布尔比如true直接与字符串相连接，那么得到的将会是1这个结果。
	$other  && ($url .= $concat.$other);
//上面这种&&的写法是表示如果前面的表达式为true，就会执行后面的表达式!!
	if($pagetype == 'admin') {
...
function getPageArr($page,$pagesize,$count,$pageurl = '',$pageajax = false) {
	...
	$pagearr = array("page"		=>$page,
					 "pagesize"	=>$pagesize,
					 "pagetotal"=>$pagetotal,
					 "limit"	=>($page - 1) * $pagesize . "," . $pagesize,//limit语句的第一个参数是开始位置，第二个参数是数量。所以这里用逗号分割的两个是开始位置加数目。
					 "count"	=>$count,
...
function getPageInfo($page,$pagetotal) {
	$from = $to = 0;
	$prev = $next = false;
	//如果是当前页为前几页。1,2,3,4那么都会是1,2,3,4,5,6如果是第五页 将会是  234567中心页与最后的页码之间是差距两个数字。中心页与第一个页码是差距三个数字。
	if($page <= 4){
			//前面几页没有没有prev。
		$prev = false;
		$next = true;
		...
		$to   = $from + 5;
		if($to>$pagetotal){
			//前面几页有个特殊情况就是总页数不足为我们要显示的数字6，，总页数不足6只能将$to设置为总页数比如4,5
			$to = $pagetotal;
			$next = false;
		}	
		//如果是最后的几页。比如一共12页，当前页是10,11,12的时候就会是7,8,9,10,11,12		
	...
	//如果不是上面的特殊情况，那么正常情况当前页-3=from，+2=to。
	if(!$from){
		$from = $page - 3;
		$to   = $page + 2;
		$prev = $next = true;
	}
	//range 是返回数组。。是从0-1将得到一个0,1的数组。
